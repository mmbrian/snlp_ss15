...
def count_transitions(taggedWords):
    ...
    lastTag = None
    for word, POStag in taggedWords:
        countPostagsPerPostag[POStag] += 1
        if lastTag:
            if POStag in countTransitionsOfPostags[lastTag]:  
                countTransitionsOfPostags[lastTag][POStag] += 1
            else: 
                countTransitionsOfPostags[lastTag][POStag] = 1
        lastTag = POStag
...
def count_emissions(taggedWords):
    ...
    for word, POStag in taggedWords:
        countWordsPerPostag[POStag] += 1
        if word in countEmissionsOfPostags[POStag]:
            countEmissionsOfPostags[POStag][word] += 1
        else:
            countEmissionsOfPostags[POStag][word] = 1
...
def get_transition_probability(previousPostag, countDataTransitions):
    ...
    probabilityTransitions = dict()
    for POS2 in countTransitionsOfPostags[previousPostag]:
        probabilityTransitions[POS2] = \
                countTransitionsOfPostags[previousPostag][POS2] / \
                countPostagsPerPostag[previousPostag]
...
if __name__ == '__main__':
    ...
    if plottingEnabled:
        def evaluate_epsilon(epsilon):
            ''' Fill this function '''
            MCR = 0
            MCR, MCRRatioOOV, sentences, logProbability = evaluate_HMM_for_epsilon(testWords, testPostags, OOVtest, epsilon, countSentences = True)
            return MCR
